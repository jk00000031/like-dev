  ...
  <header class="h-16">
    <div class="h-16 fixed top-0 left-0 z-50 w-full bg-white/90 backdrop-blur">
      <div class="container mx-auto h-full flex items-center">
        ...
        <nav class="flex-grow h-full px-4 text-xs">
          <ul class="h-full flex items-center space-x-2">
            <li class="h-full flex items-center">
              <NavItemDropdown class="h-full flex items-center" panel>
                <template #trigger>
            ...
            <li class="h-full flex items-center">
              <NavItemDropdown class="h-full flex items-center">
                <template #trigger>
                  <NuxtLink
                    to="/articles"
                    class="h-full flex items-center space-x-1 px-3 transition group-hover:text-green-600 cursor-pointer rounded"
                  >
                    <span>文章</span>
                  </NuxtLink>
                </template>
        ...
        <div ref="rightWrapperRef" class="flex items-center justify-end space-x-4">
          <template v-if="token">
            <SiteProfileHeadCard />
          </template>
          <template v-else>
